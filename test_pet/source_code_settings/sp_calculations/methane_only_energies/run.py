...
def launch(dataset, calculation, checkpoint, calculation_aux, checkpoint_aux, sp_hypers, destination, batch_size):
    ...
    os.system(f"singularity run --nv ../../singularity_pytorch_geometric/singularity_pytorch_geometric.sif python3 ../../pet_latest/estimate_error_sp.py ../../datasets/{dataset} ../../calculations/{calculation} {checkpoint} {calculation_aux} {checkpoint_aux} {sp_hypers} ../../pet_latest/default_hypers.yaml {batch_size} None False None >>{destination}")
